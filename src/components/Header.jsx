          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center">
              ...
              <button
                ...
                className="md:hidden text-blue-100 p-2 hover:bg-blue-800/30 rounded-lg transition-colors"
              >
              ...
              {/* Desktop Navigation */}
              <nav className="hidden md:flex ml-8 z-10 space-x-2">
                {Object.entries(navigation).map(([category, items]) => (
                  <div key={category} className="relative group">
                    <button
                      onMouseEnter={() => setActiveDropdown(category)}
                      onMouseLeave={() => setActiveDropdown(null)}
                      className="flex items-center z-10 px-4 py-2 text-blue-100 hover:bg-blue-800/30 rounded-lg transition-colors"
                    >
                      <span className="font-medium z-10">{category}</span>
                    ...
                    {activeDropdown === category && (
                      <div
                        onMouseEnter={() => setActiveDropdown(category)}
                        onMouseLeave={() => setActiveDropdown(null)}
                        className="absolute top-full left-0 z-10 mt-1 w-56 bg-blue-900/95 backdrop-blur-lg rounded-lg shadow-xl border border-blue-800 py-2"
                      >
                        {items.map((item, index) => (
                          <Link
                            key={index}
                            to={item.to}
                            className="flex items-center z-10 px-4 py-2.5 text-blue-100 hover:bg-blue-800/30 transition-colors"
                          >
                            <item.icon className="w-5 z-10 h-5 mr-3 text-blue-400" />
                            {item.text}
                          </Link>
                        ))}
                      </div>
